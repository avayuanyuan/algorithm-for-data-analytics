...
def compute_lower_rank_approx_via_SVD(data_matrix, desired_quality) :
    ...
    f0 = np.linalg.norm(data_matrix)
    quality = 0
    count = 0
    while quality < desired_quality:
        count = count+1
        if (count > min(data_matrix.shape[0],data_matrix.shape[1])):
            print ("no approximation, used full rank")
            count = min(data_matrix.shape[0],data_matrix.shape[1])
            break
        quality = np.linalg.norm(s[:count])/f0
    current_approximant = np.matrix(U[:,:count]) * np.diag(s[:count]) * np.matrix(V[:count,:])
...
def compute_image_block(data_matrix, k) :
    ...
    deltay = int(np.floor(1.*height/k))
    deltax = int(np.floor(1.*width/k))
    width_list = [deltax]*k
    height_list = [deltay]*k
    x_remain = width - deltax*k
    y_remain = height - deltay*k
    for i in range(x_remain):
        width_list[i] = width_list[i]+1
    for j in range(y_remain):
        height_list[j] = height_list[j]+1
    width_list = np.array(width_list)
    height_list = np.array(height_list)
    image_block = [[]for j in range(k)]
    for i in range(k):
        for j in range(k):
            startx = np.sum(width_list[:i])
            starty = np.sum(height_list[:j])
            matrix = np.matrix( data_matrix[startx:startx+width_list[i], starty:starty+height_list[j]] )
            image_block[i].append(matrix)
    return image_block
...
def get_approximation(data_matrix, k, quality, verbose) :
    ...
    blocked_data_matrix = [[]for j in range(k)]
    if verbose:
        for i in range(k):
            for j in range(k):
                myshape = image_block[i][j].shape
                h = myshape[0]
                w = myshape[1]
                print "Shape of ("+ str(i+1) + " , " +str(j+1)+") block :   (" + str(h) +  " , "+str(w) + ")"
    for i in range(k):
        for j in range(k):
            approximate = compute_lower_rank_approx_via_SVD(image_block[i][j],quality)
            blocked_data_matrix[i].append(approximate)
...
def reconstruct_data_from_image_block(image_block, k) :
    ...
    M = []
    for i in range(k):
        M.append(np.hstack(image_block[i]))
    data_matrix = np.matrix(np.vstack(M))
    return data_matrix
